...
5. Commit and push to `master` only after reviewing the diff and confirming the Marketplace setup. Watch the **Test and publish extension** GitHub Actions run and verify the resulting Marketplace listing. A push without a version bump runs checks but skips publication. If a release push did not publish, manually dispatch the workflow on master; this explicitly requests publication, while --skip-duplicate protects an existing Marketplace version.
